...
- The target data stream must use the `logs-` prefix (for example, `logs-my_dataset-default`). Targets that don't use this prefix are accepted by the endpoint but will never be indexed in {{es}}.
